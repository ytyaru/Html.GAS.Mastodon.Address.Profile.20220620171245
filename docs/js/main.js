window.addEventListener('DOMContentLoaded', async(event) => {
    ...
    document.getElementById('get-profile-from-misskey').addEventListener('click', async(event) => {
            ...
            if ('accounts' == event.detail.actions[i]) {
                ...
                if (user.hasOwnProperty('fields')) {
                    ...
                    for (let i=0; i<user.fields.length; i++) {
                        document.getElementById(`field-${i+1}-key`).value = user.fields[i].name
                        const value = document.createElement('div')
                        value.innerHTML = user.fields[i].value
